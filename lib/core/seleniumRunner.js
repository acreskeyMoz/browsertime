...
class SeleniumRunner {
  ...
  async wait(pageCompleteCheck) {
    ...
    try {
      ...
      log.error(
        `Failed waiting on page to finished loading, timed out after ${pageCompleteCheckTimeout} ms`,
        e
      );
  ...
  async loadAndWait(url, pageCompleteCheck) {
    ...
    log.info("Using webdriver.get to start pageload")
  ...
  async stop() {
    if (this.driver) {
      try {
        ...
        const bug_1298921 =
          e.message &&
          e.message.includes('Only supported in Firefox') &&
          this.options.android &&
          isAndroidConfigured(this.options);
